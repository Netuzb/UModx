...
class HelpMod(loader.Module):
    """Yordam oynasi Premium va Classic koʻrinishda"""

    strings = {
        "name": "Help",
        "bad_module": soso + "<b><b>Modul</b> <b>{}</b> <b>topilmadi.</b>",
        "single_mod_header": soso + "<b>Modul nomi</b>: {}",
        "single_cmd": "\n" + soso + "<b>{}{}</b> {}",
        "undoc_cmd": soso + "No docs",
        "premium_infor": "Maʼlumot(lar)",
        "all_header": soso + "{} MODULLAR MAVJUD - {} MODULLAR BERKITILGAN",
        "all_header_classic": soso + "<b>{} modul(lar) mavjud.\n◽ {} modul(lar) berkitilgan.</b>",
        "mod_tmpl": "\n{} <b>«{}»</b>",
        "first_cmd_tmpl": ": {}",
        "cmd_tmpl": ", {}",
        "soso_temur": soso + "️Premium-Owner: Temur",
        ...
        "partial_load": soso + "<b>Premium-Userbot</b> - faqat siz uchun.",
    ...
    strings_ru = {
        ...
        "_cls_doc": "Модуль помощи, сделанный специально для Premium-Userbot",
        "partial_load": soso + "<b>Premium-Userbot</b> - только для тебя.",
    ...
    def __init__(self):
        self.config = loader.ModuleConfig(
            ...
            loader.ConfigValue(
                "soso_emoji",
                "◽",
                lambda: "Premium-only module bullet",
                validator=loader.validators.String(length=1),
            ),
            loader.ConfigValue(
                "plain_emoji",
    ...
    async def helphidecmd(self, message: Message):
        """[modul yoki modullar] - Modullarni berkitish uchun ishlating"""
    ...
    async def modhelp(self, message: Message, args: str):
        ...
        if module.__doc__:
            reply += "\n" + soso + f"<b>{self.strings('premium_infor')}:</b> <i>" + utils.escape_html(inspect.getdoc(module)) + "\n</i>"
    ...
    @loader.unrestricted
    async def help_prcmd(self, message: Message):
        """[module] [-f] - Premium koʻrinish"""
    ...
    async def helpcmd(self, message: Message):
        ...
        reply = self.strings("all_header_classic").format(
            count,
            len(hidden) if not force else 0,
        )
        shown_warn = False
